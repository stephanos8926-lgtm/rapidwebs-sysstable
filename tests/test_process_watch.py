...
class TestFetchAllProcesses:
    def _make_mock_proc(
        self, pid, name, rss_mb, mem_pct=5.0, cpu_pct=1.0, io_read=100, io_write=50, status="running"
    ) -> MagicMock:
        proc = MagicMock()
    ...
    def test_lightweight_returns_max_20(self, mock_iter):
        procs = [self._make_mock_proc(2000 + i, f"proc_{i}", float(30 - i), mem_pct=float(30 - i)) for i in range(30)]
