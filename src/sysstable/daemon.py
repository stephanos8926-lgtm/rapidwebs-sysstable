            ...
            try:
                ...
                if pro_balance.enabled:
                    ...
                    for act in pb_actions:
                        db.save_resolution_event(
                            action=act["action"], pid=act["pid"], name=act["name"], success=True, details=act["details"]
                        )
                ...
                if fd_cfg.get("enabled", True):
                    ...
                    for snap in snaps:
                        ...
                        if pct_used >= crit_pct:
                            ...
                            if action_on_crit == "kill" and not no_kill_mgr.is_protected(
                                snap.pid, snap.name, snap.cmdline
                            ):  # noqa: E501
                                try:
                                    ...
                                    logger.warning(msg)
                                    db.save_resolution_event(
                                        action="kill_fd_limit", pid=snap.pid, name=snap.name, success=True, details=msg
                                    )
                ...
                if io_cfg.get("enabled", True):
                    ...
                    for snap in snaps:
                        matched_rule = rules_engine.match_process(snap.pid, snap.name, snap.cmdline)
                        max_read = (
                            matched_rule.get("io_read_limit_mbps", default_max_read)
                            if matched_rule
                            else default_max_read
                        )  # noqa: E501
                        max_write = (
                            matched_rule.get("io_write_limit_mbps", default_max_write)
                            if matched_rule
                            else default_max_write
                        )  # noqa: E501
                        ...
                        if read_mb > max_read or write_mb > max_write:
                            ...
                            if action_on_crit_io == "ionice":
                                try:
                                    ...
                                    logger.info(
                                        "I/O fairness: throttled I/O priority to IDLE for process %s", snap.name
                                    )  # noqa: E501
                                except Exception:  # noqa: S110
                                    pass
                            elif action_on_crit_io == "kill" and not no_kill_mgr.is_protected(
                                snap.pid, snap.name, snap.cmdline
                            ):  # noqa: E501
                                try:
                                    p = psutil.Process(snap.pid)
